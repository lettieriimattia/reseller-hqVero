...
function Waveform({ level }: { level: number }) {
  ...
  return (
    <div className="flex items-center gap-[3px] h-5">
      {factors.map((f, i) => (
        <span key={i} className="w-[3px] rounded-full bg-brand-hi transition-[height] duration-100 ease-out"
...
export default function AssistantChat({ apiCall, showToast, onAction, lang = 'it', hideBar = false, onPlus }: Props) {
  ...
  const bar = (
    <div className="relative rounded-full p-px bg-gradient-to-r from-brand/90 via-brand/90 to-brand/90 shadow-[0_8px_44px_-8px_rgba(131,151,170,0.7)]">
              ...
              <>
                ...
                {pttLocked ? (
                  <>
                    ...
                    <button onClick={() => finishPtt('send')} className="w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-br from-brand to-brand-lo text-white"><Send size={15} /></button>
                  </>
                ) : (
      ...
      <div className="flex items-center gap-2 rounded-full bg-[var(--surface-blur)] backdrop-blur-xl border border-[var(--border-2)] shadow-[0_12px_44px_-8px_rgba(0,0,0,0.6)] ring-1 ring-black/5 pl-1.5 pr-1.5 py-2">
        ...
        <button onClick={() => send(input)} disabled={sending || !input.trim()} aria-label={tx('Invia', 'Send')}
          className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 bg-gradient-to-br from-brand to-brand-lo text-white shadow-lg shadow-brand/30 transition-all hover:from-brand-hi hover:to-brand disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed">
          ...
          <div ref={panelRef} className="relative flex flex-col h-[86vh] rounded-t-[28px] border-t border-white/10 bg-[var(--surface)]/95 backdrop-blur-2xl ring-1 ring-white/5 shadow-[0_-24px_80px_-24px_rgba(131,151,170,0.55)] lg:h-[720px] lg:max-h-[80vh] lg:w-full lg:max-w-2xl lg:rounded-[24px] lg:border lg:border-white/10">
            ...
            <div className="flex items-center justify-between px-6 pt-1.5 pb-3 shrink-0">
              <div className="flex items-center gap-2.5">
                <span className="w-8 h-8 rounded-full bg-gradient-to-br from-brand to-brand flex items-center justify-center shadow-lg shadow-brand/30">
                  <Sparkles size={16} className="text-white" />
                </span>
                <div className="flex flex-col leading-none gap-1">
                  <span className="font-bold tracking-tight text-[var(--text)]">HQ<span className="text-gold">Vault</span></span>
                  <span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-hi/80">
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setWakeEnabled(v => { if (!v) userArmedRef.current = true; return !v; })} aria-label={wakeEnabled ? tx('Disattiva Ehy HQ', 'Disable Ehy HQ') : tx('Attiva Ehy HQ', 'Enable Ehy HQ')}
                  ...
                  className={`flex items-center gap-1.5 px-2.5 h-8 rounded-full text-[11px] font-bold transition-colors ${
                    wakeOn ? 'bg-brand/15 text-brand-hi ring-1 ring-brand/30'
            ...
            <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto px-5 py-3 space-y-3.5">
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[82%] px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap rounded-2xl ${
                    m.role === 'user'
                      ? 'bg-gradient-to-br from-brand to-brand-lo text-white rounded-br-md shadow-lg shadow-brand/25'
              ...
              {sending && (
                <div className="flex justify-start">
                  <div className="px-4 py-3 rounded-2xl rounded-bl-md bg-white/[0.05] border border-white/10">
                    <Loader2 size={16} className="animate-spin text-brand-hi" />
                  </div>
                </div>
              )}
              ...
              {(voiceState === 'recording' || voiceState === 'transcribing') && (
                <div className="flex justify-end">
                  <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl rounded-br-md bg-brand/15 border border-brand/30">
                    {voiceState === 'recording'
                      ? <><Waveform level={micLevel} /><span className="text-xs text-brand-hi font-medium">{tx('in ascolto…', 'listening…')}</span></>
                      : <><Loader2 size={15} className="animate-spin text-brand-hi" /><span className="text-xs text-brand-hi font-medium">{tx('trascrivo…', 'transcribing…')}</span></>}
                  </div>
                </div>
              )}
            </div>
